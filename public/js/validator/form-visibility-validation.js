window.addEventListener("load", function(){
    ...
    let form = document.querySelector("#form-visibility");
    ...
    function nameValidator () {
        ...
        if (name.value == '' || name.value == null) {
            writeMsg( { id, msg: 'El nombre del Estado no puede estar vacío' } );
            firstName.classList.add('is-invalid');
            return true
        } else if (name.value.length < 2) {
            writeMsg( { id, msg: 'El nombre del Estado debe tener al menos 2 caracteres' } );
            firstName.classList.add('is-invalid');
            return true
        }
        writeMsg( { id, msg: '' } );
