window.addEventListener("load", function(){
    ...
    let form = document.querySelector("#form-brand");
    ...
    function nameValidator () {
        ...
        if (name.value == '' || name.value == null) {
            writeMsg( { id, msg: 'El nombre de la Marca no puede estar vacío' } );
            firstName.classList.add('is-invalid');
            return true
        } else if (name.value.length < 2) {
            writeMsg( { id, msg: 'El nombre de la Marca debe tener al menos 2 caracteres' } );
            firstName.classList.add('is-invalid');
            return true
        }
        writeMsg( { id, msg: '' } );
